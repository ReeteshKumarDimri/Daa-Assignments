...
{   
	...
	inversions+=mergeSort(array,temp, begin, mid,count);
	inversions+=mergeSort(array,temp, mid + 1, end,count);
	inversions+=merge(array, temp,begin, mid +1, end,count);
	}
 	return inversions;
}
...
int main()
{
	...
	int inversions = mergeSort(a,temp, 0, arr_size - 1,comp);
	for(int i = 0; i< arr_size ; i++)
	{
    	cout<<a[i]<<" ";
